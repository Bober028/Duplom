...
function applyTheme() {
  ...
  if (isDarkMode) {
    ...
    const phoneCards = document.querySelectorAll('.phone-card');
    phoneCards.forEach(card => card.classList.add('dark-mode'));
  }
}
...
function displayPhones(phonesToDisplay) {
  ...
  phonesOnPage.forEach(phone => {
    ...
    phoneCard.innerHTML = `
      <img src="images/${phone.name.toLowerCase().replace(/\s+/g, '')}.jpg" alt="${phone.name}" class="phone-image">
      <h3>${phone.name}</h3>
      <p>${phone.brand}</p>
      <p>${phone.price} грн</p>
...
function showDetails(phoneName) {
  ...
  document.getElementById("modalImage").src = `images/${currentPhone.name.toLowerCase().replace(/\s+/g, '')}.jpg`;
